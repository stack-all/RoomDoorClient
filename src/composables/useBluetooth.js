...
export function useBluetooth() {
    ...
    const connect = async (password) => {
        ...
        try {
            ...
            challengeCharacteristic.addEventListener('characteristicvaluechanged', (event) => {
                ...
                console.log('收到新挑战:', Array.from(currentChallenge).map(b => b.toString(16).padStart(2, '0')).join(''))
            })
    ...
    const openDoor = async (password) => {
        ...
        try {
            // 使用AES加密挑战数据
            const encryptedChallenge = aesEncrypt(currentChallenge, password)
